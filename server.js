...
app.use(session({
    secret: 'super-secret-key-123', // CHANGE THIS IN PRODUCTION
    resave: false,
...
const db = new sqlite3.Database('./database.db', (err) => {
    ...
    db.serialize(() => {
        ...
        // Migrations (Ensure columns exist on restart)
        const cols = { 
...
const storage = multer.diskStorage({
    ...
    filename: (req, f, cb) => {
        // Remove spaces and special chars to prevent link breakage
...
app.use((req, res, next) => {
    ...
    if (user) {
        db.get("SELECT * FROM users WHERE id=?", [user.id], (err, dbUser) => {
            if (!dbUser) { 
                // User deleted? Log them out.
                delete req.session.users[uid]; 
                req.session.activeUserId = null; 
                return next(); 
            }
            ...
            // Notification Check (Only show newest unseen)
            db.get("SELECT * FROM notifications WHERE user_id=? AND seen=0 ORDER BY created_at DESC LIMIT 1", [user.id], (e, notif) => {
                if (notif) { 
                    res.locals.notification = notif; 
                    db.run("UPDATE notifications SET seen=1 WHERE id=?", [notif.id]); 
                }
                next();
            });
        });
...
app.post('/register', upload.single('pfpFile'), async (req, res) => {
    ...
    db.run("INSERT INTO users (username, email, password_hash, pfp_url, tos_accepted) VALUES (?, ?, ?, ?, 0)", [username, email, hash, pfp], function(err) {
        ...
        loginUser(req, {id:this.lastID, username, role:'user', pfp_url:pfp}); 
        res.redirect('/terms');
    });
});
...
app.post('/forgot-password', (req, res) => {
    ...
    db.get("SELECT * FROM users WHERE username=? AND email=?", [username, email], (err, user) => {
        ...
        db.run("UPDATE users SET reset_token=?, reset_token_expires=?, reset_attempts=0 WHERE id=?", [token, Date.now()+3600000, user.id], () => {
            transporter.sendMail({ from: '"Moaca Security" <spencer@northwinns.com>', to: email, subject: 'Reset Code', text: `Code: ${token}\nExpires in 1 hour.` });
...
app.post('/reset-password', async (req, res) => {
    db.get("SELECT * FROM users WHERE email=?", [req.body.email], async (e, u) => {
        ...
        if (u.reset_attempts >= 3) {
            db.run("UPDATE users SET reset_token=NULL WHERE id=?", [u.id]); 
            return res.render('message', { pageTitle:'Locked', type:'error', title:'Locked', message:'Too many failed attempts.' });
        }
        ...
        db.run("UPDATE users SET password_hash=?, reset_token=NULL, reset_attempts=0 WHERE id=?", [hash, u.id], () => {
            res.redirect('/login.html?success=PasswordReset');
        });
    });
});
...
app.post('/submission/delete/:id', (req, res) => {
    ...
    db.get("SELECT user_id FROM submissions WHERE id=?", [req.params.id], (e, s) => {
        if (s && (s.user_id == req.session.activeUserId || ['admin','mod'].includes(req.session.users[req.session.activeUserId].role))) {
            db.run("DELETE FROM submissions WHERE id=?", [req.params.id], ()=>res.redirect('/'));
...
app.get('/submission/:id', (req, res) => {
    db.get("SELECT * FROM submissions WHERE id=?", [req.params.id], (e,s) => {
        ...
        const isAdmin = ['admin','mod'].includes(req.session.users[req.session.activeUserId]?.role);
        if(s.status !== 'approved' && !isOwner && !isAdmin) return res.status(404).send("Not found");
...
// Edit Approvals
app.post('/admin/edit/approve/:id', (req, res) => {
    if(['admin','mod'].includes(req.session.users[req.session.activeUserId]?.role)) {
        db.get("SELECT * FROM submission_edits WHERE id=?", [req.params.id], (e, edit) => {
            if(edit) {
                db.run("UPDATE submissions SET category=?, title=?, thumbnail_url=?, description=?, download_url=?, credit_url=?, supported_versions=?, tags=?, edition=?, image_url_1=?, image_url_2=?, image_url_3=? WHERE id=?", 
                [edit.category, edit.title, edit.thumbnail_url, edit.description, edit.download_url, edit.credit_url, edit.supported_versions, edit.tags, edit.edition, edit.image_url_1, edit.image_url_2, edit.image_url_3, edit.submission_id], () => {
                    db.run("DELETE FROM submission_edits WHERE id=?", [req.params.id]); 
                    notify(edit.user_id, `Edit to "${edit.title}" Approved!`, `/submission/${edit.submission_id}`); 
                    res.redirect('/admin');
                });
            }
        });
...
// 10. PAGES & MISC
// ============================================================
const renderCat = (c, t, v, req, res) => {
    const uid = req.session.activeUserId, limit = 12, page = parseInt(req.query.page)||1, off = (page-1)*limit;
    db.get("SELECT COUNT(id) as t FROM submissions WHERE category=? AND status='approved'", [c], (e,r) => {
        ...
        db.all(sql, uid ? [uid,c,limit,off] : [c,limit,off], (e,rows) => res.render(v, {pageTitle:t, submissions:rows, currentPage:page, totalPages:Math.ceil((r?r.t:0)/limit), baseUrl:`/${v}`}));
    });
};
...
app.get('/useful', (req,res) => renderCat('useful', 'Useful', 'useful', req, res)); 
app.get('/', (req,res) => res.render('index', {pageTitle:'Home'}));
app.get('/featured', (req,res) => db.all("SELECT * FROM submissions WHERE is_featured=1 AND status='approved'", (e,rows) => res.render('featured', {pageTitle:'Featured', submissions:rows})));
app.get('/information', (req,res) => res.render('information', {pageTitle:'Info'}));
...
app.get('/submission', (req,res) => res.render('submission', {pageTitle:'Submit'})); 
app.get('/register.html', (req,res) => res.render('register', {pageTitle:'Register'})); 
